		...
		shownWindow = new SimpleObjectProperty<>(event -> {
			...
			if (id != null && id.getValue() != 0) {
				...
				for (Measurement elem : current.getMeasurements()) {
					MeasurementViewModel m = allMeasurements.getValue().stream().filter(p -> p.getId() == elem.getId()
					).findFirst().get();
		...
		saveCommand = new DelegateCommand(() -> new Action() {
			...
			protected void action() throws Exception {
				...
				if (idTemp.getValue() == 0) {
					...
					dbHelper.updateCharacteristic(typeId, idTemp.getValue(), codeTemp.getValue(), nameTemp.getValue(),
							measurementIdList);
				}
